...
Обновление версии — явная правка тега в `package.json` + переустановка +
...
**После правки тега — `npm install @prf/ui`, не голый `npm install`.**
Проверено на практике: обычный `npm install` иногда не перерезолвливает уже
закреплённую в lock-файле git-зависимость даже при изменении тега в
`package.json` — переустановка конкретного пакета форсирует обновление
достоверно. Проверить результат: `npm ls @prf/ui` должен показать новый SHA.
